...
These standards keep Sweet Paws understandable, safe to change, and consistent as the product grows. They apply to application code, tests, Firebase configuration, and documentation.
...
- Components render and coordinate interaction; they do not contain scattered Firestore SDK calls.
- Use custom hooks for reusable UI/data behaviour and repositories for persistence.
...
- Repository interfaces use domain-oriented inputs and outputs. Firebase types, paths, and document mapping remain inside repository implementations.
- Security Rules are mandatory for access control; client checks are never treated as authorisation.
- Every new Firestore query documents its intended bounds and required index/rules coverage.
...
- Use Firebase emulators for repository/security-rule tests where practical.

## Dependencies and architecture changes

- Prefer native platform and existing stack capabilities before adding a package.
- Any significant dependency, managed service, or new Firebase product needs a documented problem statement, security/privacy/cost assessment, and entry in `TechDecisions.md`.
- Do not add a custom backend or Cloud Function merely to proxy ordinary Firestore CRUD.
